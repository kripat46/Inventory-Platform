...
# ------------------------------------------------------------ 
            system_prompt = (
                ...
                "CONVERSATIONAL SAFE-HARBOR EXCEPTION: Standard professional platform greetings, connection checks, casual user alerts "
                "(such as 'Hello', 'Hi', 'running dry on stuff'), and platform tests are explicitly ALLOWED. For generic courtesies, "
                "simply respond with a brief, professional greeting text block and you are STRICTLY FORBIDDEN from calling the 'stage_mitigation_parameters' tool function.\n"
                "VALID ENTERPRISE TOPICS & FILES: You are explicitly ALLOWED to process, read, and analyze all internal unstructured data files "
                "including: incident report batches, competitor price wars, marketing calendar files, supplier disruptions, and warehouse audits. These are valid logistics inputs.\n"
                "If the user's intent falls completely outside of retail operations or these whitelisted topics, you must immediately halt execution and respond "
